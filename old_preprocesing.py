...
def add_word_embeddings(X_train: pd.DataFrame, X_test: pd.DataFrame) -> tuple([pd.DataFrame, pd.DataFrame]):
    ...
    X_train = pd.concat([X_train.reset_index(drop=True), scores_eval.reset_index(drop=True)], axis = 1)
    X_test = pd.concat([X_test.reset_index(drop=True), scores_eval.reset_index(drop=True)], axis=1)
